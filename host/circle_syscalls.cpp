//
// circle_syscalls.cpp — OpenTyrian's file access, routed to the core that
// owns the hardware.
//
// THE PROBLEM. OpenTyrian reads its data files, its configuration and its
// saved games with plain C stdio: fopen, fread, mkdir. Those reach newlib,
// and newlib's Circle glue drives FatFs and the SD card directly. On this
// kernel the game runs on the application core, and touching a device from
// any core but core 0 is illegal — the whole Circle world, interrupts
// included, belongs to core 0. OpenTyrian has no file layer of its own to
// redirect, so there is nothing in the game to change.
//
...
// The game's own output — every printf it makes, on descriptors 1 and 2
