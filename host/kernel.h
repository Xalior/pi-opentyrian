//
// kernel.h — the Circle kernel that hosts OpenTyrian.
//
// Device ownership: this kernel brings up interrupts, the timer, a serial
// console carrying stdio, the SD card (FatFs, holding the Tyrian data files,
// the configuration and the saved games), the USB host controller and the
// cooperative scheduler. Video and audio belong to circle-libsdl2 and are
// created inside SDL_Init when OpenTyrian calls it. So do the CPU clock and
// the case fan, which the shim manages for its host; this kernel only says
// when they come up.
//
...
//   core 1   the APPLICATION core. OpenTyrian, alone: the game loop, the
//            OPL synthesis and nothing else. Every platform call it makes
