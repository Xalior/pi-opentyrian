//
// kernel.cpp — brings the board up, then hands control to OpenTyrian.
//
// OpenTyrian is an ordinary command-line program: it expects a working
// standard library, a filesystem holding its Tyrian data files, and an SDL2
...
// starts a core; electing one is the host's job, and this is where it
// happens. OpenTyrian itself knows none of it: it calls plain SDL, and its
// file access reaches the marshalled I/O service through the syscall layer
// in circle_syscalls.cpp.
//
#include "kernel.h"
#include "defaults.h"
...
// the Circle kernel. Declared extern "C" because OpenTyrian is C: once
// renamed it is still a C function with a C symbol name.
...
// and not part of the SDL one. OpenTyrian is keyboard-only, so this is the
// whole of its control surface when no USB keyboard is attached.
class CSerialDevice;
void SDL2Circle_SetInjectSerial(CSerialDevice *pSerial);

static const char From[] = "opentyrian";

// OpenTyrian's own command line. Everything it needs is compiled in or read
// from its configuration file, so nothing is baked here beyond the program
// name; a boot adds to this through the defaults block.
static const char *OpenTyrianArgv[] = {
    "opentyrian",
};
...
// terminating null.
static const char *s_FinalArgv[sizeof(OpenTyrianArgv) / sizeof(OpenTyrianArgv[0])
...
void CSplitCores::Run(unsigned nCore)
{
    ...
    case 1:
        ...
        s_AppResult = opentyrian_main(s_FinalArgc, const_cast<char **>(s_FinalArgv));
...
// Monotonic across releases, always a plausible "now". OpenTyrian seeds its
// random number generator from time(), so this has to be a real answer.
...
{
    m_Logger.Write(From, LogNotice, "starting OpenTyrian");
    ...
    // mode. It never sets what the game is given — that is the declaration
    // below.
    ...
    // library about the display. Every SDL answer OpenTyrian gets — the
    ...
    //
    // 640x400 is Tyrian's 320x200 VGA raster at exactly double size. That is
    // what OpenTyrian's default scaler (Scale2x) produces, so its own
    // SDL_RenderCopy is 1:1 and the library's is the only scale in the chain.
    // It also lets the plain 320x200 scaler double cleanly if the player picks
    // it. It is a constant because it is a fact about the program, not a
    // setting.
    //
    // The library has no default and no fallback: without this it refuses to
    // start.
    static const int VIRTUAL_WIDTH  = 640;
    static const int VIRTUAL_HEIGHT = 400;
    ...
    s_FinalArgc = DefaultsBuildArgv(OpenTyrianArgv,
    ...
    else
    {
        ...
        res = opentyrian_main(s_FinalArgc, const_cast<char **>(s_FinalArgv));
    }
    ...
    m_Logger.Write(From, LogNotice, "OpenTyrian exited with %d — parked", res);
    for (;;)
    {
